...
class Microscopy:
    ...
    class MicroscopyWarning(BreadWarning):
    ...
    def from_tiff(filepath: Path) -> 'Microscopy':
        ...
        # self.calibration_factor = self.get_calibration_factor_tif(data)
    ...
    def from_nd2(filepath: Path, field_of_views: List[int] = None, channels: List[int] = None) -> 'Microscopy':
        ...
        with ND2Reader(str(filepath)) as images:
            ...
            # self.calibration_factor = get_calibration_factor_nd2(images)

        return microscopies , channel_names

    def get_calibration_factor_nd2(nd2):
        metadata = nd2.metadata
        if 'pixel_microns' in metadata:
            # Check if 'pixel_microns' key is present in metadata
            calibration_factor = metadata['pixel_microns']
            return calibration_factor
        else:
            MicroscopyWarning("Calibration information not found in the metadata.")
            return None
    
    def get_calibration_factor_tif(tif):
        ...
        if x_resolution is not None and y_resolution is not None:
            ...
            x_resolution_value = x_resolution.value[1] / x_resolution.value[0]
            y_resolution_value = y_resolution.value[1] / y_resolution.value[0]
            
            if x_resolution_value == y_resolution_value == 1.0:
                # in this case, the resolution is not specified trust worthy, it's uncommon that the resolution is 1.0, so we assume it's not specified
                return None
            if x_resolution_value == y_resolution_value:
                # Assuming resolution is in micrometers per pixel
                return x_resolution_value
            else: 
                MicroscopyWarning("X and Y resolution are different, please check the calibration.")
        else:
            ...
            MicroscopyWarning("Calibration information not found in the specified tags.")
            return None
...
class Segmentation:
    ...
    class SegmentationWarning(BreadWarning):
        def __init__(self, message) -> None:
            super().__init__(f'{message}')
    ...
    def replace_frame(self, frame_number: int, new_frame: np.ndarray):
        """Replace a frame in the segmentation with a new frame.

        Parameters
        ----------
        frame_number : int
            Index of the frame to be replaced.
        new_frame : np.ndarray
            New frame to replace the existing frame.
        """
        if frame_number < 0 or frame_number >= len(self):
            print(f'frame_number {frame_number} out of range for segmentation with {len(self)} frames.')
            raise ValueError("Invalid frame number")

        if new_frame.shape != self.data[frame_number].shape:
        ...
        try:
            ...
            for fov in groups:
                if file[fov].keys():
                    ...
                    load_frames = list(range(np.max(numeric_times)+1)) # load all frames, frames start from zero, so we need to add one

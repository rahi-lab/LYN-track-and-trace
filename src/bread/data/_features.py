...
from ._constants import train_set_avg_cell_diameter
...
class Features:
	...
	segmentation: Segmentation
	scale_length: float = field(init=False, repr=True) # [length unit]/px
	scale_time: float = 1.0 # [time unit]/frame
	budding_time: int = 5
	...
	def __post_init__(self):
		self.scale_length = train_set_avg_cell_diameter / self.segmentation.average_cell_diameter_pixel

	def replace_frame_in_segmentation(self, frame_id: int, new_frame: np.ndarray):
		...
		self._cached_ellipses = {}
		self._cached_ellipses_max_ecc = {}
		self._cached_cms = {}
		self.segmentation.replace_frame(frame_id, new_frame)
